...
public class ResetData {
	...
	public void resetData() {
		...
		Statement runQueries;
		try {
			runQueries = connection.createStatement();
			runQueries.execute(dropTableQ);
			runQueries.execute(createTablesQ);
			runQueries.execute(insertDataQ);

		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("Failed resetting database");
		}
	}
}
